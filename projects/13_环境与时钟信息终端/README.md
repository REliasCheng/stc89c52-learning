...
这是仓库中的个人综合实践，目标是把已经出现过的DS1302、DS18B20、OLED、独立按键和AT24C02接口组织成一个完整应用，而不是重新复制五套驱动。应用层负责页面、按键事件、刷新节拍和配置状态；C51适配层把这些接口接到现有课程驱动。
...
当前C51入口使用100 ms前台节拍，便于先验证多模块数据流，没有占用Timer0。DS1302独立走时；DS18B20读取会等待转换。迁移到非阻塞版本时，可由Timer0只产生系统tick，再把温度转换拆成“启动”和“稍后读取”两个状态。
...
测试覆盖配置读取、周期采样、页面切换、偏移调整和显式保存。C51适配代码已检查函数与文件依赖；当前环境没有Keil C51，因此没有声明板端构建和实机运行结果。
